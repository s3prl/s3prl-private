...
MAX_AMP = 0.9
# In hidden_set all the sources are at 16K Hz
RATE = 16000
...
parser.add_argument('--hidden_set_md_dir', type=str, required=True,
                    help='Path to hidden_set metadata directory')
...
def main(args):
    ...
    hidden_set_md_dir = args.hidden_set_md_dir
    ...
    if md_dir is None:
        root = os.path.dirname(hidden_set_dir)
        md_dir = os.path.join(root, f'LibriMix/metadata')
    os.makedirs(md_dir, exist_ok=True)
    create_librimix_metadata(hidden_set_dir, hidden_set_md_dir, wham_dir,
                             wham_md_dir, md_dir, n_src)


def create_librimix_metadata(hidden_set_dir, hidden_set_md_dir, wham_dir,
                             wham_md_dir, md_dir, n_src):
    """ Generate LibriMix metadata according to hidden_set metadata """

    # Dataset name
    dataset = f'libri{n_src}mix'
    # List metadata files in hidden_set
    hidden_set_md_files = os.listdir(hidden_set_md_dir)
    ...
    for hidden_set_md_file in hidden_set_md_files:
        if not hidden_set_md_file.endswith('.csv'):
            print(f"{hidden_set_md_file} is not a csv file, continue.")
            continue
        # Get the name of the corresponding noise md file
        try:
            wham_md_file = [f for f in wham_md_files if
                            (hidden_set_md_file.split('-')[0] in f)][0]
            print(wham_md_file)
            print(hidden_set_md_file)
        except IndexError:
            print('Wham metadata are missing you can either generate the '
                  'missing wham files or add the hidden_set metadata to '
                  'to_be_ignored list')
            break

        # Open .csv files from hidden_set
        hidden_set_md = pd.read_csv(os.path.join(
            hidden_set_md_dir, hidden_set_md_file), engine='python')
        ...
        save_path = os.path.join(md_dir,
                                 '_'.join([dataset, hidden_set_md_file]))
        info_name = '_'.join([dataset, hidden_set_md_file.strip('.csv'),
                              'info']) + '.csv'
        ...
        mixtures_md, mixtures_info = create_librimix_df(
            hidden_set_md, hidden_set_dir, wham_md, wham_dir,
            n_src)
...
def create_librimix_df(hidden_set_md_file, hidden_set_dir,
                       wham_md_file, wham_dir, n_src):
    """ Generate librimix dataframe from a hidden_set and wha md file"""
    ...
    pairs, pairs_noise = set_pairs(hidden_set_md_file, wham_md_file, n_src)
    clip_counter = 0
    ...
    for pair, pair_noise in tqdm(zip(pairs, pairs_noise), total=len(pairs)):
        ...
        sources_info, sources_list_max = read_sources(
            hidden_set_md_file, pair, n_src, hidden_set_dir)
        # Add noise
...
def set_pairs(hidden_set_md_file, wham_md_file, n_src):
    """ set pairs of sources to make the mixture """
    # Initialize list for pairs sources
    utt_pairs = []
    noise_pairs = []
    # In train sets utterance are only used once
    if 'train' in hidden_set_md_file.iloc[0]['subset']:
        utt_pairs = set_utt_pairs(hidden_set_md_file, utt_pairs, n_src)
        noise_pairs = set_noise_pairs(utt_pairs, noise_pairs,
                                      hidden_set_md_file, wham_md_file)
...
def set_utt_pairs(hidden_set_md_file, pair_list, n_src):
    # A counter
    c = 0
    # Index of the rows in the metadata file
    index = list(range(len(hidden_set_md_file)))
    ...
    while len(index) >= n_src and c < 200:
        ...
        speaker_list = set([hidden_set_md_file.iloc[couple[i]]['speaker_ID']
                            for i in range(n_src)])
...
def set_noise_pairs(pairs, noise_pairs, hidden_set_md_file, wham_md_file):
    ...
    for pair in pairs.copy():
        # get sources infos
        sources = [hidden_set_md_file.iloc[pair[i]]
...
def read_sources(metadata_file, pair, n_src, hidden_set_dir):
    ...
    for i in range(n_src):
        ...
        absolute_path = os.path.join(hidden_set_dir,
                                     source['origin_path'])
...
def set_loudness(sources_list):
    ...
    # Initialize loudness
    loudness_list = []
    # In hidden_set all sources are at 16KHz hence the meter
