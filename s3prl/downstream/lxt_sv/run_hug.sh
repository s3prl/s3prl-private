...
if [ -z "$*" ]; then
    lrs=("1" "1.0e-1" "1.0e-2" "1.0e-3" "1.0e-4")
else
    lrs=($*)
fi
...
do
    ...
    do
        ...
        python3 run_downstream.py --upstream_feature_normalize -a -m train -u $org/$repo --upstream_revision $revision -s ASV -d lxt_sv -o config.optimizer.lr=$lr \
