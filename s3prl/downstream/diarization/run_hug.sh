...
do
    ...
    python3 run_downstream.py --upstream_feature_normalize -a -m train -u $org/$repo --upstream_revision $revision -s SD -d diarization -o config.optimizer.lr=$lr \
        -p $expdir --hub huggingface -c downstream/diarization/lxt.yaml
