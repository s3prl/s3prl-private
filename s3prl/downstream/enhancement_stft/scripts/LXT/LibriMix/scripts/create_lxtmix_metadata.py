...
parser.add_argument('--lxt_dir', type=str, required=True,
                    help='Path to LXT root directory')
parser.add_argument('--lxt_md_dir', type=str, required=True,
                    help='Path to LXT metadata directory')
...
parser.add_argument('--n_repeat', type=int, required=True,
                    help='Number of times to repeat each audio')


def main(args):
    lxt_dir = args.lxt_dir
    lxt_md_dir = args.lxt_md_dir
    ...
    create_librimix_metadata(lxt_dir, lxt_md_dir, wham_dir,
                             wham_md_dir, md_dir, n_src)


def create_librimix_metadata(lxt_dir, lxt_md_dir, wham_dir,
                             wham_md_dir, md_dir, n_src):
    ...
    for lxt_md_file in lxt_md_files:
        if not lxt_md_file.endswith('.csv'):
            print(f"{lxt_md_file} is not a csv file, continue.")
            continue
        # Get the name of the corresponding noise md file
        try:
            wham_md_file = [f for f in wham_md_files if
                            f.startswith(lxt_md_file.split('.')[0])][0]
        except IndexError:
            print('Wham metadata are missing you can either generate the '
                  'missing wham files or add the lxt metadata to '
                  'to_be_ignored list')
            break

        # Open .csv files from LibriSpeech
        lxt_md = pd.read_csv(os.path.join(
            lxt_md_dir, lxt_md_file), engine='python')
        ...
        save_path = os.path.join(md_dir,
                                 '_'.join([dataset, lxt_md_file]))
        info_name = '_'.join([dataset, lxt_md_file.split('.')[0],
                              'info']) + '.csv'
        ...
        mixtures_md, mixtures_info = create_librimix_df(
            lxt_md, lxt_dir, wham_md, wham_dir,
            n_src)
...
def create_librimix_df(lxt_md_file, lxt_dir,
    ...
    pairs, pairs_noise = set_pairs(lxt_md_file, wham_md_file, n_src)
    clip_counter = 0
    cnt = 0
    ...
    for pair, pair_noise in tqdm(zip(pairs, pairs_noise), total=len(pairs)):
        ...
        sources_info, sources_list_max = read_sources(
            lxt_md_file, pair, n_src, lxt_dir, cnt)
        # Add noise
...
def set_pairs(lxt_md_file, wham_md_file, n_src):
    """ set pairs of sources to make the mixture """
    # Initialize list for pairs sources
    utt_pairs = []
    noise_pairs = []
    # In train sets utterance are only used once
    if 'train' in lxt_md_file.iloc[0]['subset']:
        utt_pairs = set_utt_pairs(lxt_md_file, utt_pairs, n_src, n_repeat=n_src * args.n_repeat)
        noise_pairs = set_noise_pairs(utt_pairs, noise_pairs,
                                      lxt_md_file, wham_md_file)
        #utt_pairs, noise_pairs = remove_duplicates(utt_pairs, noise_pairs)
    else:
        utt_pairs = set_utt_pairs(lxt_md_file, utt_pairs, n_src, n_repeat=n_src)
        noise_pairs = set_noise_pairs(utt_pairs, noise_pairs,
                                      lxt_md_file, wham_md_file)
        utt_pairs, noise_pairs = remove_duplicates(utt_pairs, noise_pairs)
    return utt_pairs, noise_pairs

def set_utt_pairs(lxt_md_file, pair_list, n_src, n_repeat=1):
    # A counter
    c = 0
    # Index of the rows in the metadata file
    index = list(range(len(lxt_md_file)))
    index *= n_repeat
    ...
    while len(index) >= n_src and c < 200:
        ...
        speaker_list = set([lxt_md_file.iloc[couple[i]]['speaker_ID']
                            for i in range(n_src)])
...
def set_noise_pairs(pairs, noise_pairs, lxt_md_file, wham_md_file):
    ...
    for pair in pairs.copy():
        # get sources infos
        sources = [lxt_md_file.iloc[pair[i]]
...
def read_sources(metadata_file, pair, n_src, lxt_dir, cnt):
    ...
    for i in range(n_src):
        ...
        absolute_path = os.path.join(lxt_dir,
                                     source['origin_path'])
