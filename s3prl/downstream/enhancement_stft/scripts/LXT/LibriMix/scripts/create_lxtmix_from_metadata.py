...
parser.add_argument('--lxtmix_outdir', type=str, default=None,
...
parser.add_argument('--n_hours', type=int, default=5,
...
def main(args):
    ...
    lxtmix_outdir = args.lxtmix_outdir
    n_src = args.n_src
    ...
    create_lxtmix(lxt_dir, wham_dir, lxtmix_outdir, metadata_dir,
                    freqs, n_src, modes, types)


def create_lxtmix(lxt_dir, wham_dir, out_dir, metadata_dir,
                    freqs, n_src, modes, types):
    """ Generate sources mixtures and saves them in out_dir"""
    # Get metadata files
    md_filename_list = [file for file in os.listdir(metadata_dir)
                        if 'info' not in file]
    # Create all parts of lxtmix
    for md_filename in md_filename_list:
        csv_path = os.path.join(metadata_dir, md_filename)
        process_metadata_file(csv_path, freqs, n_src, lxt_dir,
                              wham_dir, out_dir, modes, types)


def process_metadata_file(csv_path, freqs, n_src, lxt_dir, wham_dir,
                          out_dir, modes, types):
    ...
    for freq in freqs:
        ...
        for mode in modes:
            ...
            dir_name = os.path.basename(csv_path).replace(
                f'LXT{n_src}mix_', '').replace('-clean', '').replace(
                '.csv', '')
            ...
            process_utterances(md_file, lxt_dir, wham_dir, freq, mode,
                               subdirs, dir_path, subset_metadata_path, n_src)


def process_utterances(md_file, lxt_dir, wham_dir, freq, mode, subdirs,
    ...
    for results in tqdm.contrib.concurrent.process_map(
        functools.partial(
            process_utterance, 
            n_src, lxt_dir, wham_dir, freq, mode, subdirs, dir_path),
