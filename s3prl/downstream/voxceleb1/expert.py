...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, expdir, **kwargs):
        ...
        if kwargs["use_extracted_feature"] and kwargs["mode"] != "extract":
            assert (
                os.path.exists(self.extract_kwargs["extracted_path"] / "train") and
                os.path.exists(self.extract_kwargs["extracted_path"] / "dev") and
                os.path.exists(self.extract_kwargs["extracted_path"] / "test")
        ...
        extracted_kwargs = self.extract_kwargs.copy()
        extracted_kwargs.pop("mode")
    ...
    def _get_train_dataloader(self, dataset):
        ...
        return DataLoader(
            ...
            prefetch_factor=self.datarc.get('prefetch_factor', 2),
    ...
    def forward(self, mode, features, labels, filenames, records, **kwargs):
        ...
        features_len = torch.tensor([len(feat) for feat in features], dtype=torch.int, device=device)
