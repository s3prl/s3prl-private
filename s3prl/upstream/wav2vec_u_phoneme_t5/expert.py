...
class UpstreamExpert(UpstreamBase):
    def __init__(self, joint_dict, ppg=False, hidden=False, **kwargs):
        ...
        self.T5 = PhonemeT5(self.u_downsample_rate)
        ...
        import logging
        logging.info("feature_groups: {}".format(self.feature_groups))
        if ppg:
            self.feature_groups["hidden_states"].append([self.feature_num])
            self.feature_num += 1
            logging.info("feature_num: {}".format(self.feature_num))
        self.feature_groups["hidden_states"].append([self.feature_num])
        import logging
        logging.info("feature_groups: {}".format(self.feature_groups))
