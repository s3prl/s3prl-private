...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def _trim_or_cat(self, fbank_lengths, old_out):
        ...
        old_out = old_out[:, 1:].view(old_out.shape[0], seq_len, mel_len, old_out.shape[-1]).flatten(2, 3)
        new_out = []
        cls_idx = [0]
    ...
    def forward(self, wavs):
        ...
        fbank_lengths = []
        for wav in wavs:
            fbank = self._wav2fbank(wav)
            fbank_lengths.append(fbank.shape[0])
            fbanks.extend(self._split_fbank(fbank))
        ...
        seq_outs, scene_outs = zip(*map(partial(self._trim_or_cat, fbank_lengths), outs))
        ...
        return {
            ...
            "ASV": scene_outs,
            "ER": scene_outs,
            "feature_lengths": [ceil(l / patch_size[1]) * (NUM_MEL_BINS // patch_size[1]) for l in fbank_lengths]
        }